...
# Six GRPO updates on one repeated stateful-tool prompt. Reward is the exact
# final token B after a valid two-call trace. Use a fresh RUN_ID unless resuming.

RUN_ID="${RUN_ID:-r1}"
EXP_NAME="sciprobe_rl_lightning-overfit-canary-${RUN_ID}"
...
RUN_ROOT="${PERSISTENT_ROOT}/runs/lightning-overfit-canary-${RUN_ID}"
...
export COMMAND="export PATH=/opt/uv/bin:/opt/nemo_rl_venv/bin:\${PATH} && \
  ...
  uv run --locked --no-sync python ./examples/nemo_gym/run_grpo_nemo_gym.py \
    ...
    logger.wandb.name=lightning-overfit-canary-${RUN_ID} \
  ...
  uv run --locked --no-sync python examples/validate_sciprobe_overfit_canary_outputs.py \
    --run-root ${RUN_ROOT} --model ${MODEL_PATH} --config ${CONFIG_PATH} \
    --expected-max-steps 6 --expected-train-rollouts 16 \
...
echo "probe_id=stateful-choice-overfit-v1"
