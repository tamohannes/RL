...
"""Create a private runtime copy of the one-row overfit dataset."""
...
def main() -> None:
    ...
    if len(rows) != 1 or not isinstance(rows[0], dict):
        raise RuntimeError("overfit source must contain exactly one JSON object")
    row = rows[0]
    if row.get("id") != PROBE_ID or row.get("probe_id") != PROBE_ID:
        raise RuntimeError("overfit source has the wrong probe id")
    forbidden_fields = {"_sciprobe_verifier_capability", "sciprobe_capability"}
    if forbidden_fields.intersection(row):
        raise RuntimeError("overfit dataset must not persist a verifier capability")
