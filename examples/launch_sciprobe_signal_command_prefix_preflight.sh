...
# CPU-only rehearsal of the exact signal-canary command prefix. It starts the
# production Ray and sandbox sidecars, then runs every gate before model loading.

RUN_ID="${RUN_ID:-r1}"
EXP_NAME="sciprobe_rl_signal-prefix-preflight-${RUN_ID}"
...
OUTPUT_ROOT="${OUTPUT_ROOT:-${PERSISTENT_ROOT}/preflights/signal-command-prefix-${RUN_ID}}"
...
export COMMAND="export PATH=/opt/uv/bin:/opt/nemo_rl_venv/bin:\${PATH} && \
  ...
  echo '[SCIPROBE_SIGNAL_PREFIX_PREFLIGHT_OK]'"
